...
CREATE OR REPLACE DATABASE EXEC;
USE DATABASE EXEC;

CREATE OR REPLACE SCHEMA EXEC;
USE SCHEMA EXEC;
...
CREATE OR REPLACE PROCEDURE execute_students_query()
  RETURNS TABLE (NAME STRING, AGE INT, PHONE_NO STRING, EMAIL STRING)
  LANGUAGE SQL
AS
$$
DECLARE
  test RESULTSET DEFAULT (
  SELECT NAME, AGE, PHONE_NO, EMAIL
  FROM STUDENTS
  WHERE AGE = 20);
BEGIN
RETURN TABLE(test);
...
-- Note: The CALL syntax used should be compatible with Snowflake SQL
CALL execute_students_query();
